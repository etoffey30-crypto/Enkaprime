/*
  Migration 010: Add `image_url` column to `programmes` and provide
  example upserts / updates so the Admin UI can show thumbnails when you
  upload images to Supabase Storage.

  Usage:
  1) Upload image files to a public bucket in Supabase Storage (e.g. bucket
     name `programmes`).
  2) Use the template below to set `image_url` for each programme by code.
     Replace `<project-ref>` and `<bucket>` / `<filename>` with your values.

  Public URL template (replace placeholders):
    https://<project-ref>.supabase.co/storage/v1/object/public/<bucket>/<filename>

  Run this file in the Supabase SQL editor (Primary Database) to add the
  column and to perform example updates.
*/

-- 1) Add the image_url column (idempotent)
ALTER TABLE programmes
  ADD COLUMN IF NOT EXISTS image_url text DEFAULT '';

-- 2) Example: set image_url for specific programmes after uploading files
-- Replace <project-ref>, <bucket>, and <filename> with your project values.
-- Example URL: https://abc123xyz.supabase.co/storage/v1/object/public/programmes/LMT100.jpg

-- Example update for a single programme:
-- UPDATE programmes
--   SET image_url = 'https://<project-ref>.supabase.co/storage/v1/object/public/programmes/LMT100.jpg', updated_at = now()
-- WHERE code = 'LMT 100';

-- 3) Optional: upsert all June programmes with empty image_url placeholders
INSERT INTO programmes (code, title, days, category, is_active, is_featured, description, image_url)
VALUES
...
ON CONFLICT (code) DO UPDATE
  SET
    ...
    image_url = COALESCE(EXCLUDED.image_url, programmes.image_url),
    updated_at = now();

-- 4) After running the above, upload image files to Storage and run UPDATE
--    statements to set proper URLs for each `code` so the admin UI shows
--    thumbnails.
